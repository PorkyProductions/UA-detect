...
/**
 * Checks whether the browser supports data sharing using the `canShare` API.
 * @param data - The data to be shared.
 * @returns `true` if the browser supports data sharing, `false` otherwise.
 */
export const canShareData = (data?: ShareData): boolean => {
	/**
   * Firefox does not support the `canShare` API.
   */
	if (browser === 'Firefox') {
		...
		/**
     * Uses the browser's `canShare` method to check if data can be shared.
     */
		const result = getterForNavigator().canShare(data);

		/**
     * Returns true if the `canShare` method returns true, otherwise returns false.
     */
		return result === true;
	}
};
